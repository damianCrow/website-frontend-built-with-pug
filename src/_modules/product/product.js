...
export default class Product {
  constructor() {
  	...
  	/* Copyright (c) 2006-2013 Tyler Uebele * Released under the MIT license. * latest at https://github.com/tyleruebele/details-shim * minified by Google Closure Compiler */
		...
		$('.product-detail__label').click((e) => {
			if($(e.target).hasClass('open')) {
				$(`details.${$(e.target).attr('name')}`).prop('open', false);
				$(e.target).removeClass('open').addClass('closed').html('Open All');
			} else {
				$(`details.${$(e.target).attr('name')}`).prop('open', true);
				$(e.target).removeClass('closed').addClass('open').html('Close All');
			}
		})
	}
}
